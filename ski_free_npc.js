(() => {
  ...
  let autoplay = {
    ...
    run: function() {
      ...
      if (Math.random() < 0.02) { // Occasionally change lanes
        const currentLane = laneX.indexOf(game.player.x);
        const newLane = Math.max(0, Math.min(laneX.length - 1, currentLane + (Math.random() > 0.5 ? 1 : -1)));
