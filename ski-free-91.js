(() => {
  ...
  let autoplay = {
    ...
    run: function() {
      ...
      // Simple autoplay logic - move left/right based on NPC skier position
      const npcSkiers = game.npcSkiers;
      if (npcSkiers.length > 0) {
        const closestSkier = npcSkiers.reduce((closest, skier) => {
          return skier.y > closest.y ? skier : closest;
        });

        // Move left or right based on NPC skier position
        if (closestSkier.x < canvas.width / 2) {
