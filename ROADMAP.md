...
- [ ] Add a dog spawn timer that activates when game state is PLAYING
- [ ] Implement spawnDog() to create a new Dog instance at a random x position on the slope
- [ ] Ensure spawnDog() only triggers during PLAYING state and respects the timer interval
