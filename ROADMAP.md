...
- [!] Add drone spawn logic that generates drones at random intervals above the player's current y-position <!-- blocked: multi-entity system (spawn+move+collision+draw) — same insert-only-coder assembly ceiling as dogs. Deferred by maintenance to keep shipping single-function items; reopen via hand-build or a fine-tuned coder. -->
