...
- [!] Create a function spawnDog() that adds a new dog object with random x, y set to -NPC_SKIER_HEIGHT, and dead: false to game.dogs only when game.state is PLAYING  <!-- blocked: pipeline: item INCOMPLETE — 0/4 landed, 0 unspecifiable, 2 e -->
- [!] Add a dog spawn timer that activates when game state is PLAYING <!-- blocked: multi-function dog assembly (init/spawn/wire-timer/update/draw/collision) exceeds the insert-only coder even WITH bge-3 grounding — functions land but do not wire coherently, game.dogs undefined at boot. Deferred by maintenance to ship self-contained Phase-3 features; reopen via hand-build or a fine-tuned coder. -->
- [!] Implement spawnDog() to create a new Dog instance at a random x position on the slope <!-- blocked: multi-function dog assembly (init/spawn/wire-timer/update/draw/collision) exceeds the insert-only coder even WITH bge-3 grounding — functions land but do not wire coherently, game.dogs undefined at boot. Deferred by maintenance to ship self-contained Phase-3 features; reopen via hand-build or a fine-tuned coder. -->
- [!] Ensure spawnDog() only triggers during PLAYING state and respects the timer interval <!-- blocked: multi-function dog assembly (init/spawn/wire-timer/update/draw/collision) exceeds the insert-only coder even WITH bge-3 grounding — functions land but do not wire coherently, game.dogs undefined at boot. Deferred by maintenance to ship self-contained Phase-3 features; reopen via hand-build or a fine-tuned coder. -->
- [!] Add function updateDogs() that moves each dog in game.dogs down-slope at NPC_SKIER_SPEED and steers its x toward the player x each frame to chase; wire it into the update loop <!-- blocked: multi-function dog assembly (init/spawn/wire-timer/update/draw/collision) exceeds the insert-only coder even WITH bge-3 grounding — functions land but do not wire coherently, game.dogs undefined at boot. Deferred by maintenance to ship self-contained Phase-3 features; reopen via hand-build or a fine-tuned coder. -->
- [!] Add function drawDogs() that draws each dog in game.dogs as an ORIGINAL code-drawn dog shape and palette, distinct from the skier; wire it into the render path <!-- blocked: multi-function dog assembly (init/spawn/wire-timer/update/draw/collision) exceeds the insert-only coder even WITH bge-3 grounding — functions land but do not wire coherently, game.dogs undefined at boot. Deferred by maintenance to ship self-contained Phase-3 features; reopen via hand-build or a fine-tuned coder. -->
- [!] Add function checkDogCollision() that, on player overlap with a dog in game.dogs, briefly SLOWS the player with a short speed penalty (not a hard crash) then marks the dog dead; wire it into the update loop <!-- blocked: multi-function dog assembly (init/spawn/wire-timer/update/draw/collision) exceeds the insert-only coder even WITH bge-3 grounding — functions land but do not wire coherently, game.dogs undefined at boot. Deferred by maintenance to ship self-contained Phase-3 features; reopen via hand-build or a fine-tuned coder. -->
- [ ] Add drone spawn logic that generates drones at random intervals above the player's current y-position
- [ ] Implement drone movement: horizontally track the player with a slow, smooth offset based on distance
- [ ] Add drone collision detection with the player, triggering a crash state on contact
- [ ] Draw the drone using original pixel-art style: a small, stylized flying machine with rotating propellers
